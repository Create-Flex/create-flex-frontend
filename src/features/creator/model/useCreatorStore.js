...
export const useCreatorStore = create((set, get) => ({
  ...
  // 크리에이터 목록 조회 (검색 및 페이징 지원)
  fetchCreators: async (name = null, page = 0, size = 10) => {
    set({ isLoading: true, error: null });
    try {
      const { mapCreatorFromBackend } = await import('../../../shared/utils/creatorMapper');
      const response = await creatorService.getAllCreators(name, page, size);
