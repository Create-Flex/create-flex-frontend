...
import { Megaphone, Plus, DollarSign, Ban, CheckCircle2, User as UserIcon, ChevronLeft, ChevronRight } from 'lucide-react';
import { advertisementService } from '../../api/advertisementService';
import { useAdvertisementStore } from '../../model/useAdvertisementStore';
import { mapAdvertisementFromBackend } from '../../../../shared/utils/advertisementMapper';
import {
...
export const AdsTab = ({
    ...
    const fetchAdvertisements = async (filter, page = 0) => {
        setLoading(true);
        setError(null);
        try {
            const backendFilter = filter === 'pending' ? 'waiting' : filter === 'history' ? 'processed' : 'all';
            const response = await advertisementService.getMyAdvertisements(backendFilter, page, 8);

            console.log('받아온 광고 데이터:', response);

            // Page 객체 대응 (content, totalPages 등)
            const content = response.content || [];
            const mappedAds = content.map(mapAdvertisementFromBackend);
    ...
    useEffect(() => {
        setCurrentPage(0);
        fetchAdvertisements(adFilter, 0);
    }, [adFilter]);

    useEffect(() => {
        if (refreshTrigger) {
            fetchAdvertisements(adFilter, currentPage);
        }
    }, [refreshTrigger]);

    const handlePageChange = (newPage) => {
        if (newPage >= 0 && newPage < totalPages) {
            fetchAdvertisements(adFilter, newPage);
        }
    };
    ...
    const handleAdDecision = async (adId, decision) => {
        ...
        try {
            ...
            await fetchAdvertisements(adFilter, currentPage);
        } catch (error) {
