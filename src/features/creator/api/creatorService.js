...
export const creatorService = {
  ...
  // 업무 목록 조회
  getCreatorWorks: async (creatorId) => {
    try {
      const response = await api.get(`/creators/${creatorId}/works`, {
        headers: {
          'Accept': 'application/json; charset=UTF-8'
        }
      });

      console.log('크리에이터 업무 목록 응답:', response.data);
      return response.data;
    } catch (error) {
      console.error('크리에이터 업무 목록 조회 에러:', error);
      throw error;
    }
  },

  // 업무 추가
  createCreatorWork: async (creatorId, workName) => {
    try {
      const response = await api.post(`/creators/${creatorId}/works`, {
        workName
      }, {
        headers: {
          'Content-Type': 'application/json; charset=UTF-8'
      ...
      console.log('크리에이터 업무 추가 성공:', response.data);
      return response.data;
    } catch (error) {
      console.error('크리에이터 업무 추가 에러:', error);
      throw error;
    }
  },

  // 업무 상태 변경 (WORKING ↔ DONE)
  updateCreatorWorkStatus: async (creatorId, workId, workStatus) => {
    try {
      const response = await api.patch(`/creators/${creatorId}/works/${workId}`, {
        workStatus
      }, {
        headers: {
          'Content-Type': 'application/json; charset=UTF-8'
      ...
      console.log('크리에이터 업무 상태 변경 성공:', response.data);
      return response.data;
    } catch (error) {
      console.error('크리에이터 업무 상태 변경 에러:', error);
      throw error;
    }
  },

  // 업무 삭제
  deleteCreatorWork: async (creatorId, workId) => {
    try {
      const response = await api.delete(`/creators/${creatorId}/works/${workId}`, {
        headers: {
          'Content-Type': 'application/json; charset=UTF-8'
        }
      });

      console.log('크리에이터 업무 삭제 성공:', response.data);
      return response.data;
    } catch (error) {
      console.error('크리에이터 업무 삭제 에러:', error);
      throw error;
    }
  }
};
